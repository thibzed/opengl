...
class Planet{
    ...
    public:
        Planet (const std::vector<float>& r0, const std::vector<float>& v0, float radius,
                const std::vector<float>& color, bool compute_orbit,
                const char* vertexShader = "../shaders/sphere/sphere.vs",
                const char * fragmentShader = "../shaders/sphere/sphere.fs") : 
        _r(r0), _v(v0), _radius(radius), _orbitFlag(compute_orbit),
    ...
    private: 
        ...
        void setup_init_condition(){
            ...
            if(_orbitFlag){
                ...
                int percentage = 50;
                _orbit.push_back(_r[0]);
                _orbit.push_back(_r[1]);
                _orbit.push_back(_r[2]);
